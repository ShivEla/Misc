...
Original file is located at
    https://colab.research.google.com/drive/1szE_39_Kqi7eKyd8lvHJYedLzl2H48wA
"""
...
G_from_file = nx.read_edgelist('edges.txt',create_using=nx.DiGraph())
...
G_from_gml = nx.read_gml('dolphins.gml', label='id')
print("Nodes:", G_from_gml.nodes())
print("Edges:", G_from_gml.edges())

# Draw the graph
nx.draw(G_from_gml, with_labels=True, node_color='lightpink', font_weight='bold')
plt.show()

import networkx as nx
import pandas as pd
from pprint import pprint # pprint is used for "pretty-printing" dictionaries

def get_all_metrics(G):
    """
    Calculates a dictionary of all key metrics for a given graph G.
    """
    metrics = {}
    ...
    # Check for connectivity before calculating metrics that require it
    ...
    # Note: These return dictionaries of {node: value}
    metrics['degree_centrality'] = nx.degree_centrality(G)
    metrics['betweenness_centrality'] = nx.betweenness_centrality(G)
    metrics['closeness_centrality'] = nx.closeness_centrality(G)
    metrics['degree_histogram'] = nx.degree_histogram(G)

    return metrics

# --- Example Usage ---
# Create a sample graph to test the function
G_sample = nx.karate_club_graph()

# Get all the metrics by calling your custom function
all_graph_metrics = get_all_metrics(G_sample)

# Print the results neatly
print("--- All Graph Metrics ---")
pprint(all_graph_metrics)
